...
> **注意：**陣列驅動通常使用在[測試](/docs/{{version}}/testing)上，用來防止 session 資料持續存在。
...
首先來取得 session。可以在控制器裡的方法，經由型別提示的 HTTP 請求取得 session 實例。請記得，控制器方法的依賴是透過 Laravel 的[服務容器](/docs/{{version}}/container)注入：
...
要加入額外驅動至 Laravel 的後端 session 中，你可以使用 `Session` [facade](/docs/{{version}}/session) 的 `extend` 方法。你可以在[服務提供者](/docs/{{version}}/providers) 的 `boot` 方法內呼叫 `extend` 方法：
