...
在過去，開發者得為每個需要排程的任務產生 Cron 項目。而最令人頭疼的是任務排程不受版本控制，並且你需要 SSH 到你的伺服器增加 Cron 項目。Laravel 指令調度器（command scheduler）允許你清楚流暢的在 Laravel 當中定義排程指令，並且只需要在你的伺服器上面增加一條 Cron 項目。
...
你的排程已經被定義在 `app/Console/Kernel.php` 檔案的 `schedule` 方法中。為了方便你開始，一個簡單的範例已經包含在該方法中。你可以自由的增加工作排程到 `Schedule` 物件當中。
...
這裡是唯一需要加入到伺服器的 Cron 項目：
Here is the only Cron entry you need to add to your server:

    * * * * * php /path/to/artisan schedule:run >> /dev/null 2>&1


該 Cron 將於每分鐘呼叫 Laravel 指令調度器，接著 Laravel 會評定你的排程任務並且執行預定的任務。
This Cron will call the Laravel command scheduler every minute. Then, Laravel evaluates your scheduled tasks and runs the tasks that are due.

<a name="defining-schedules"></a>
## Defining Schedules

你可以將所有已排定的任務定義在 `App\Console\Kernel` 類別的 `schedule` 方法中。一開始，讓我們看一個排程任務的範例。在該範例，我們將排定一個在午夜被呼叫的敝包。該敝包將執行清除表格的資料庫查詢：
...
除了排定 `敝包` 呼叫，你還你排定 [Artisan 命令](/docs/{{version}}/artisan) 以及作業系統指令。舉個例子，你可以使用 `command` 方法排定一個 Artisan 指令：
...
`->everyTenMinutes();`  |  於十分鐘執行該任務
`->everyThirtyMinutes();`  |  於每三十分鐘執行該任務
`->hourly();`  |  於每小時鐘執行該任務
`->daily();`  |  於每天午夜執行該任務
`->dailyAt('13:00');`  |  於每天 13:00 執行該任務
`->twiceDaily();`  |  於每天 1:00 以及 13:00 執行該任務
`->weekly();`  |  於每週執行該任務
`->monthly();`  |  於每月午夜執行該任務
Method  | Description
------------- | -------------
...
這些方法可以合併其他限制條件，藉以產生更精細的排程，例如在能在一週的某幾天執行的排程。舉個例子，排定一個每週的星期一的排程：
...
`when` 方法可以被用於限制任務是根據給定的為真驗證。換句話說，如果給定的 `敝包` 回傳 `true`，這個任務將持續被執行只要沒有其他的限制條件。
...
預設情況，排定的任務將被執行，即便之前相同的任務仍未結束。為了避免這個問題，你可以使用 `withoutOverlapping` 方法：
By default, scheduled tasks will be run even if the previous instance of the task is still running. To prevent this, you may use the `withoutOverlapping` method:

    $schedule->command('emails:send')->withoutOverlapping();

在這個範例，如果非執行中，`emails:send` [Artisan 指令](/docs/{{version}}/artisan) 將於每分鐘執行。如果你有些超長執行時間的任務，而不知道確切的執行時間，`withoutOverlapping` 方法將特別有幫助。
...
Laravel 調度器為任務排程輸出提供許多方便的方法。首先，透過 `sendOutputTo` 可以發送輸出到單一檔案做為事後檢驗：
...
透過 `emailOutputTo` 方法，你可以發送輸出到你所選的電子郵件。注意，你必須先透過 `sendOutputTo` 方法輸出到一個檔案。當然，在將任務的輸出發送到電子郵件之前，你需要先設定 Lravel 的 [電子郵件服務](/docs/{{version}}/mail)：
...
透過 `pingBefore` 與 `thenPing` 方法，調度器能自動地在一個任務完成之前或之後 ping 一個給定的網址。該方法在你排定的任務進行或完成時，通知一個外部的服務顯得非常有效，像是 [Laravel Envoyer](https://envoyer.io)。
